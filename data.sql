...
//장바구니 아이템 목록 조회
SELECT cartsitems.id, book_id, title, summary, quantity,price FROM cartsitems LEFT JOIN books ON cartsitems.book_id = books.id;

//장바구니 아이템 조회
DELETE FROM cartsitems WHERE id=?;

//장바구니에서 선택한 아이템 목록 조회
SELECT * FROM Bookshop.cartsitems WHERE user_id=1 AND id IN (1,3)
